...
// falcoin: optional setting to unlock wallet for block minting only;
...
// falcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // falcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // falcoin: sub-cent change is moved to fee
...
// falcoin: create coin stake transaction
...
// falcoin: check 'spent' consistency between wallet and txindex
// falcoin: fix wallet spent state according to txindex
...
// falcoin: disable transaction (only for coinstake)
